...
Like Java rules, rules_scala includes the entire transitive classpath when compiling. Practically, scalac [requires the entire classpath](https://github.com/scalacenter/advisoryboard/blob/master/proposals/009-improve-direct-dependency-experience.md).

Bazel pushes towards explicit and minimial dependencies to keep BUILD file hygiene and allow targets to refactor their dependencies without fear of downstream breaking.
To that end, strict dependency checking verifies that all direct dependencies of code are direct dependencies in Bazel.
To learn more about the motivation of strict-deps, see [the Bazel blog post](https://blog.bazel.build/2017/06/28/sjd-unused_deps.html) on the subject.

To use strict-deps, add `--strict_java_deps=WARN|ERROR` to your `bazel` invocation.
